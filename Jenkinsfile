...
pipeline {
    ...
    environment {
        ...
        // Never hardcode the AWS account ID: derived at runtime below.
        ACCOUNT_ID        = ''
        ECR_REGISTRY      = ''
        IMAGE_TAG         = ''
        ALB_DNS_NAME      = ''
        PREV_FRONTEND_TASKDEF = ''
        PREV_BACKEND_TASKDEF  = ''
    }
        ...
        stage('Checkout') {
            steps {
                ...
                script {
                    ...
                    env.ALB_DNS_NAME = sh(
                        script: "aws elbv2 describe-load-balancers --region ${env.AWS_REGION} --names ${env.PROJECT_NAME}-${env.ENVIRONMENT}-alb --query 'LoadBalancers[0].DNSName' --output text",
                        returnStdout: true
                    ).trim()
